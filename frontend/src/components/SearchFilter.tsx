...
const SearchFilter = ({ setData, setError, error }: SearchFilterProps) => {
  ...
  useEffect(() => {
    ...
    [selectedPerson, setData, setError])
  ...
  return (
    <>
      ...
      {
        error === "error color" && <ErrorMessage message="Sua pesquisa não encontrou nenhuma tarefa com a cor correspondente." />
      }
      {
        error === "error favorite" && <ErrorMessage message="Sua pesquisa não encontrou nenhuma tarefa marcado como favorito." />
      }
    </>
  )
}
